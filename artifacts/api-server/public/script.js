(() => {
  ...
  function nextDateISO(d) { const x = new Date(); x.setDate(x.getDate() + d); return localDateStr(x); }
  ...
  function migrate(s) {
    ...
    s.revisions = (s.revisions || []).filter(r => r.subId && r.chId && r.tId).map(r => ({
      id: r.id || uid(),
      subId: r.subId, chId: r.chId, tId: r.tId,
      completedAt: r.completedAt || todayKey(),
      schedule: (r.schedule || []).filter(st => typeof st.offset === 'number' && st.dueDate).map(st => ({
  ...
  function scheduleRevisionsForTopic(subId, chId, tId) {
    ...
    const today = todayKey();
    const existing = state.revisions.find(r => r.subId === subId && r.chId === chId && r.tId === tId);
    if (existing) {
  ...
  function checkGoalCompletions() {
    ...
    for (const g of state.goals || []) {
      ...
      if (p.isComplete && !g.completedAt) {
        ...
        changed = true;
        setTimeout(() => {
          toast(`Goal reached: ${goalDisplayName(g)} 🎯`, 'success');
        }, 100);
  ...
  function modalSetReminderTime(which, index) {
    ...
    openModal(`
      ...
      <div class="actions" style="margin-top:16px">
        ...
        <button class="btn" id="tp-save" type="button">${isAdd ? 'Add Time' : 'Save'}</button>
      </div>
    `, () => {
      const root = document.getElementById('modal-root');
  ...
  function dueLabel(daysOverdue, dueDate) {
    const dateSuffix = dueDate ? ` (${formatDate(dueDate)})` : '';
    if (daysOverdue === 0) return `Due today${dateSuffix}`;
    if (daysOverdue === 1) return `1 day overdue${dateSuffix}`;
    return `${daysOverdue} days overdue${dateSuffix}`;
  }
  ...
  function renderActivity(days) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      counts.push(state.activity[localDateStr(d)] || 0);
  ...
  function renderActivityWeeks(weeks) {
    ...
    for (let w = weeks - 1; w >= 0; w--) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        total += state.activity[localDateStr(dt)] || 0;
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    switch (act) {
      ...
      case 'menu-subject': { e.stopPropagation(); const _ms = findSubject(subId); if (_ms) openDropdown(actEl, subjectMenu(_ms)); break; }
      case 'menu-chapter': { e.stopPropagation(); const _ms2 = findSubject(subId); const _mc = findChapter(subId, chId); if (_ms2 && _mc) openDropdown(actEl, chapterMenu(_ms2, _mc)); break; }
      case 'menu-topic':   { e.stopPropagation(); const _ms3 = findSubject(subId); const _mc2 = findChapter(subId, chId); const _mt = findTopic(subId, chId, tId); if (_ms3 && _mc2 && _mt) openDropdown(actEl, topicMenu(_ms3, _mc2, _mt)); break; }
